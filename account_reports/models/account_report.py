...
class AccountReport(models.Model):
    ...
    # Those fields allow case-by-case fine-tuning of the engine, for custom report.
    ...
    def _create_menu_item_for_report(self):
        """ Adds a default menu item for this report. This is called by an action on the report, for report created manually by the user.
        """
        self.ensure_one()
    ...
    def _get_options_journals(self, options):
        ...
        if not selected_journals:
            ...
            # This is needed, because some report will not use ALL available journals and filter by type.
    ...
    def _multi_company_tax_units_init_options(self, options, previous_options=None):
        """ Initializes the companies option for report configured to compute it from tax units.
        """
        ...
        if previous_options and previous_options.get('tax_unit') in companies_authorized_tax_unit_opt:
            ...
            # (which should always be true for non-generic report, which have a country), and the companies of
    ...
    def _init_options_columns(self, options, previous_options=None):
        ...
        options['show_debug_column'] = options['export_mode'] != 'print' \
                                       ...
                                       and len(self.line_ids) > 0 # No debug column on fully dynamic report by default (they can customize this)
    ...
    def _init_options_variants(self, options, previous_options=None):
        ...
        for variant in all_variants.filtered(lambda x: x._is_available_for(options)):
            if not self.root_report_id and variant != self and variant.active: # Non-route report don't reroute the variant when computing their options
    ...
    def _custom_options_add_integer_rounding(self, options, integer_rounding, previous_options=None):
        """ Helper function to be called in a _custom_options_initializer by report needing to use the integer_rounding feature.
    ...
    def _get_generic_line_id(self, model_name, value, markup='', parent_line_id=None):
        ...
        - markup:  a (possibly empty) string allowing finer identification of the line
                   (like the name of the field for account.accounting.report)
        ...
        for line_dict in line_dict_list:
            for column_dict in line_dict['columns']:
                ...
                if 'name' in column_dict and not force_format:
                    ...
                    # This gives additional flexibility to custom report, if needed.
                    continue
                    ...
                    if expression.report_line_id.report_id == self:
                        if expression in column_group_expression_totals:
                            ...
                            raise UserError(_(
                                ...
                                "Make sure the cross-report aggregations of this report only reference terms belonging to other report.",
    ...
    def _compute_totals_no_batch_aggregation(self, column_group_options, formulas_dict, other_current_report_expr_totals, other_cross_report_expr_totals_by_scope):
        ...
        :param other_cross_report_expr_totals: A dict(forced_date_scope, expression_totals), where expression_totals is in the same form as
                                               ...
                                               report than self. The forced_date_scope corresponds to the original date_scope set on the
    ...
    def _generate_default_external_values(self, date_from, date_to, is_tax_report=False):
        ...
        If is_tax_report, the values are only created for tax report, else for all other report.
        """
        options_dict = {}
        ...
        # Get all the default expressions from all report
        default_expressions = self.env['account.report.expression'].search([('label', '=like', '_default_%')])
        # Options depend on the report, also we need to filter out tax report/other report depending on is_tax_report
    ...
    def _get_column_headers_render_data(self, options):
        ...
        # since custom report could alter them (e.g. for multi-currency purposes)
        ...
        # Custom report have the possibility to define custom subheaders that will be displayed between the generic header and the column names.
    ...
    def _get_load_more_line(self, offset, parent_line_id, expand_function_name, groupby, progress, options):
        ...
        :param progress: A json-formatted dict(column_group_key, value) containing the progress value for each column group, as it was
                         returned by the expand function. This is for example used by report such as the general ledger, whose lines display a c
    ...
    def _inject_report_into_xlsx_sheet(self, options, workbook, sheet):
        ...
        # For report with lines generated for accounts, the account name and codes are shown in a single column.
    ...
    def _add_options_xlsx_sheet(self, workbook, options_list):
        ...
        # Try to find the common filter values between all report to avoid duplication.
...
class AccountReportHorizontalGroup(models.Model):
    ...
    _description = "Horizontal group for report"
...
class AccountReportHorizontalGroupRule(models.Model):
    ...
    _description = "Horizontal group rule for report"
...
class AccountReportCustomHandler(models.AbstractModel):
    ...
    # This abstract model allows case-by-case localized changes of behaviors of report.
    # This is used for custom report, for cases that cannot be supported by the standard engines.

    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        """ Generates lines dynamically for report that require a custom processing which cannot be handled
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        """ When using the 'unfold all' option, some report might end up recomputing the same query for
