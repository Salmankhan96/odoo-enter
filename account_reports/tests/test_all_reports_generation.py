...
class TestAllReportsGeneration(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        # Make the report always available, so that they don't clash with the comany's country
        cls.reports.availability_condition = 'always'
        # We keep the country set on each of these report, so that we can load the proper test data when testing exports

    def test_open_all_reports(self):
        # 'unfold_all' is forced on all report (even if they don't support it), so that we really open it entirely
        ...
        for report in self.reports:
            with self.subTest(report=report.name, country=report.country_id.name):
                ...
                if report.use_sections:
                    self.assertNotEqual(options['report_id'], report.id, "Composite report should always reroute.")
    ...
    def test_generate_all_export_files(self):
        # Test values for the fields that become mandatory when doing exports on the report, depending on the country
        ...
        # Some root report are made for just one country and require test fields to be set the right way to generate their exports properly.
        # Since they are root report and are always available, they normally have no country set; we assign one here (only for the ones requiring it)
