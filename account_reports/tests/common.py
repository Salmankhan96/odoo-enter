...
class TestAccountReportsCommon(AccountTestInvoicingCommon):
    ...
    def assertIdenticalLines(self, reports):
        """Helper to compare report lines with the same `code` across multiple report.
        ...
        :param reports: (recordset of account.report) The report to check
        """
        ...
        if not reports:
            raise UserError('There are no report to compare.')
            ...
            with self.subTest(line_code=line.code):
                for tested_line in identical_lines:
                    self.assertCountEqual(
                        ...
                        (
                            f'The line with code {line.code} from report "{line.report_id.name}" and '
                            f'"{tested_line.report_id.name}" has different expression values in both report.'
                        )
                    )
